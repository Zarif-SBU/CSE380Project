...
export default class SlimeAttack extends NPCAction {
                ...
                effects: [
                    {
                        ...
                        end: this.actor.position.x + 1.3*(this.target.position.x - this.actor.position.x),
                        ease: EaseFunctionType.IN_OUT_QUAD
                    },
                    {
                        property: TweenableProperties.posY,
                        start: this.actor.position.y,
                        end: this.actor.position.y + 1.3*(this.target.position.y - this.actor.position.y),
                        ease: EaseFunctionType.IN_OUT_QUAD
                    }
                ]
            });
            this.actor.tweens.play("attack");
            (this.target.position.x - this.actor.position.x) < 0? this.actor.animation.play("Attack_Left") : this.actor.animation.play("Attack_Right");
    ...
    //         if ((Math.pow(this.target.position.x - this.actor.position.x, 2) + Math.pow(this.target.position.y - this.actor.position.y, 2)) < 30004) {
    ...
    //             }
    //             // this.path = this.actor.getPath(this.actor.position, this.target.position);
    //             if(Math.pow(this.target.position.x - this.actor.position.x, 2) + Math.pow(this.target.position.y - this.actor.position.y, 2) > 30000) {
    ...
    protected checkOverlap() {
        ...
        let disty = this.actor.position.y - this.actor.position.y;
        if(distx > this.actor.collisionShape.hw + 32) {
            return false;
        }
        if(disty > this.actor.collisionShape.hh + 64) {
            return false;
        }
        // console.log("hit");
        return true;
