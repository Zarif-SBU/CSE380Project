...
export default abstract class PlayerState extends State {
    ...
    public override update(deltaT: number): void {
        if((<Sprite>this.owner).getLastVelocity().x != 0) {
            facing = (<Sprite>this.owner).getLastVelocity().x;
        }
        ...
        if(Input.isJustPressed("DODGE") && !this.owner.animation.isPlaying("BOOST_RIGHT") && !this.inAction()){
            ...
            //     }
            // } else {
                this.parent.owner.move(this.parent.controller.moveDir.scale(14));
            // }
        }
	...
	inAction () {
        ...
        let box1:AABB = new AABB(new Vec2(start.x+(facing*120), start.y+(dir.y*120)), new Vec2(60, 60))
